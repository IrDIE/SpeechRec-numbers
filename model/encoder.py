...
class ConformerCTC(nn.Module):
    ...
    def decode(self, log_probs, lengths):
        """Delegate to active decoder. Returns list[list[str]] of words per sample.

        Args:
            log_probs: (B, T, V) log-softmax outputs from get_log_probs.
            lengths:   (B,) true encoder output lengths per sample.
        """
        if self.decoder is None:
            raise RuntimeError("No decoder set. Use set_decoder() or pass one to __init__.")
